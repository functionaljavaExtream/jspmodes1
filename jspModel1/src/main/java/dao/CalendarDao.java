...
public class CalendarDao {
	...
	public CalendarDto getCalendarDetail(int seq) {
		String sql = "select seq, id, title, content, rdate, wdate "
				+ "	from calendar "
				+ "	where seq=?";
		...
		try {
			conn = DBConnection.getConnection();
			System.out.println("1/3 getCalendarDetail success");
			psmt = conn.prepareStatement(sql);
			psmt.setInt(1, seq);
			System.out.println("2/3 getCalendarDetail success");
			rs = psmt.executeQuery();
			System.out.println("3/3 getCalendarDetail success");
			if(rs.next()) {
